...
if csv_files:
   ...
   #            f"DE_project.product_staging_table "\
    ...
    if data:
        ...
        logger.info("no record match") #if record is missing in CSV file
...
#logging.info(msg: "File path available on s3 under %s bucket and folder name is %s", "args: bucket_name,file_paths")
...
try:
    ...
    logger.error("File download_error: %s", e)
    sys.exit()

# Get a list of all files in the total directory
...
logger.info("Please stop csv files that needs to be processed %s", csv_files)
